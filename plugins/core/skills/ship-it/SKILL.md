...
- **Nudge to finalize the `docs/` journal before push** — if this branch has a `docs/issues/` or `docs/features/` doc (see `/document`), remind the user to record final state and outcomes. Soft reminder surfaced in step 4's follow-ups, never a blocking gate.
