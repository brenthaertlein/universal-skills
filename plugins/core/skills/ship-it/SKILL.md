...
- "Finalize the `docs/` journal — record final state and outcomes in `docs/issues/` or `docs/features/` (see `/document`) before this merges"
...
- **Nudge to finalize the `docs/` journal before wrapping up** — if this branch has a `docs/issues/` or `docs/features/` doc (see `/document`), remind the user to record final state and outcomes. Surfaced as a step 4 follow-up (after the push), never a blocking gate.
